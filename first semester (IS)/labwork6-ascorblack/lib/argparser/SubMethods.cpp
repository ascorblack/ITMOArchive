...
bool ArgParser::IsActiveArgument(const std::string& arg_name) const {
  ArgumentVariant argument_variant;
  GetArgument(arg_name, argument_variant);

  switch (argument_variant.arg_type) {
    case ArgType::String:
      return argument_variant.string_argument->IsActive();
    case ArgType::Int:
      return argument_variant.int_argument->IsActive();
    case ArgType::Flag:
      return argument_variant.flag_argument->IsActive();
    case ArgType::Undefined:
      break;
  }
}

bool ArgParser::FinishWithHelp() {
  return help_ != nullptr ? help_->finish_with_help : false;
}
