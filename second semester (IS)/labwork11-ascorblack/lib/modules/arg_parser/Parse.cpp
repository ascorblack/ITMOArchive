...
void ArgParser::ParseMultiValueArgument(ArgumentVariant& arguments_variant, int& index, const std::vector<std::string>& args) {
  ...
  if (arguments_variant.arg_type == ArgType::String) {
    arguments_variant.string_argument->SetActive(true);
    ...
    if (arguments_variant.string_argument->GetArgumentStoreType() == ArgStorageType::Multi) {
      for (++index; index < args.size(); ++index) {
        ...
        arguments_variant.string_argument->AddValue(args[index]);
      }
    } else {
      ++index;
      if (args[index].starts_with('-')) {
        ...
        if (arguments_variant.string_argument->GetDefault().has_value()) {
          ...
          throw std::runtime_error("Not found positional argument value!");
      }

      arguments_variant.string_argument->AddValue(args[index]);
    }
  } else if (arguments_variant.arg_type == ArgType::Int) {
    arguments_variant.int_argument->SetActive(true);
    ...
    if (arguments_variant.int_argument->GetArgumentStoreType() == ArgStorageType::Multi) {
      for (++index; index < args.size(); ++index) {
        ...
        arguments_variant.int_argument->AddValue(StringToNumber(args[index]));
      }
    }
  }
}
