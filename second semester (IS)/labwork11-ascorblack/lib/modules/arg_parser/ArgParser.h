...
struct ParserHelp {
  ...
  bool finish_with_help = false;

  explicit ParserHelp() = default;
  explicit ParserHelp(char short_name, const char* long_name, const char* description);

  std::string HelpDescription(const std::unordered_map<std::string, ArgumentVariants>&, const char*&) const;
...
class ArgParser {
 public:
  ...
  bool IsActiveArgument(const std::string&) const;
